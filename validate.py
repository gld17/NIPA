...
def cal_qcn(model_name, pim_params, device_only=False):
	...
	for i in range(wl_weight-1):
		...
		if vari_type == 'state_dependent':
			...
			if vari > 0:
				device_error_distribution = norm(loc=0, scale=math.sqrt(subArray*vari))
				device_factor = scipy.integrate.quad(device_vari_function,0, -adc_step_size/2, adc_step_size/2, args=device_error_distribution)[0]
				device_factor /= vari*subArray
				device_factor = max(1-device_factor,0)
			else:
				device_factor = 1
...
def main():
		...
		for line in lines:
			if 'wl_weight' in line:
				...
				if pim_params not in pim_params_list:
					...
					acc_repeat=True
			if 'Acc' in line and not acc_repeat:
	...
	for i in range(len(_acc_list)):
		i_wl_weight = pim_params_list[i]['wl_weight']
		i_adc_precision = pim_params_list[i]['ADCprecision']
		i_rram_vari = pim_params_list[i]['vari']
		i_subArray = pim_params_list[i]['subArray']
		i_cellbit = pim_params_list[i]['cellBit']
		# if pim_params_list[i]['SAF_prop']>0:
		for j in range(len(_acc_list)):
			if i<j:
				j_wl_weight = pim_params_list[j]['wl_weight']
				j_adc_precision = pim_params_list[j]['ADCprecision']
				j_rram_vari = pim_params_list[j]['vari']
				j_subArray = pim_params_list[j]['subArray']
				j_cellbit = pim_params_list[j]['cellBit']
				if i_rram_vari==j_rram_vari==0 and i_adc_precision==j_adc_precision and i_subArray==j_subArray and i_cellbit==j_cellbit==1:
					if (i_wl_weight>=j_wl_weight)!=(_acc_list[i]>=_acc_list[j]):
						remove_index.append(i)
						remove_index.append(j)
				# if i_wl_weight==j_wl_weight and i_adc_precision==j_adc_precision and i_subArray==j_subArray and i_cellbit==j_cellbit==1:
				# 	if (i_rram_vari>=j_rram_vari)!=(_acc_list[i]>=_acc_list[j]):
				# 		remove_index.append(i)
				# 		remove_index.append(j)
	...
	np.savetxt(os.path.join(params.path, '{}_nipa_results.txt'.format(params.dataset)), final_results)
	...
	plt.grid(True)
	fig.tight_layout()
	fig.savefig(os.path.join(params.path, '{}_nipa_fitting.png'.format(params.dataset)), dpi=600)
