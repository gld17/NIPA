...
class Buffer(object):
	def __init__(self, buf_capacity):
		# buffer technology unit: nm
		self.buf_Tech = 28 # default 28nm

		# buffer size unit: KB
		self.buf_Size = buf_capacity

		# buffer bitwidth unit: bit
		self.buf_bitwidth = 256
		...
		self.buf_area = self.calculate_area()
	...
	def calculate_read_power(self):
		...
		return self.buf_rpower

	def calculate_write_power(self):
		# unit: mW
		if self.buf_Size == 0:
			self.buf_wpower = 0
		else:
		...
		return self.buf_wpower


	def calculate_read_latency(self, rdata):
		# unit: ns, Byte(data)
		self.buf_rlatency = math.ceil(rdata*8/self.buf_bitwidth)*self.buf_cycle

		return self.buf_rlatency

	def calculate_write_latency(self, wdata):
		# unit: ns, Byte(data)
		self.buf_wlatency = math.ceil(wdata*8/self.buf_bitwidth)*self.buf_cycle

		return self.buf_wlatency

	def calculate_read_energy(self, rdata):
		# unit: nJ
		...
		return self.buf_renergy

	def calculate_write_energy(self, wdata):
		# unit: nJ
...
def buf_test():
	...
	_buf.calculate_area()
	_buf.calculate_read_power()
	_buf.calculate_read_latency()
	_buf.calculate_read_energy()
	_buf.calculate_write_power()
	_buf.calculate_write_latency()
	_buf.calculate_write_energy()
	_buf.buf_output()
